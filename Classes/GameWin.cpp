...
bool GameWin::init() {
	...
	auto background = Sprite::create("GameWinBG.png");
	...
	Score->setPosition(visibleSize.width / 2, visibleSize.height / 5 * 2 - 30);
	this->addChild(Score);
	...
	MenuItemSprite* TryAgain = MenuItemSprite::create(TryAgainNormal, nullptr,
		CC_CALLBACK_1(GameWin::onNewGame, this));

	MenuItemSprite* Back = MenuItemSprite::create(BackNormal, nullptr,
		CC_CALLBACK_1(GameWin::BacktoMain, this));
...
void GameWin::onNewGame(Ref* pSender)
{
	Scene* scene = Scene::create();
...
void GameWin::BacktoMain(Ref* pSender)
{
